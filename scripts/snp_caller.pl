...
# ignore file can specify regions to be ignored
...
#	print  "Reading: ".$vcffile."\n" if $verbose;
	...
	unless ( -d $outdir.'/snp_filter/' ) {
		mkdir $outdir.'/snp_filter/';
	}
	
	my $vcf_out = $outdir.'/snp_filter/'.$vcf_outfile.'.txt';
	...
	# variables to deal with regions with no sites called
	my $position_index = 0;
	my $missing_sites;
...
my $combined_data;  # will store the SNPs for phylip matrix
my $position_data;
...
foreach my $seq ( sort keys %{$data} ) {
	...
	my $data_counter1 = 0;  # counts total number of positions processed
	my $data_counter2 = 0;  # counts number of positions after skipping missing/uncertain sites
	...
	foreach my $pos ( sort { $a <=> $b } keys %{$data->{$seq}} ) {
		...
		foreach my $vcf ( @vcffiles ) {
			...
			if ( $data->{$seq}->{$pos}->{$vcf} ) {			
				...
				# make sure some het notation hasn't slipped through
				die "\nThis shouldn't happen: ".$pos." :: ".$alt."\n" if $alt =~ m/,.+$/;
				
				# check that the ref agrees with the ref collected from the vcf, otherwise something has gone wrong
				...
				# the call in this vcf must be an alt
				$base_call = $alt;
			} else { 
				# if not called as variant, uncertain or gap, must be ref 
				print  "\tBASE CALL: ".$seq.','.$pos.','.$vcf."\t".$data->{$seq}->{$pos}->{$vcf}."\n" if $verbose > 2;
				$base_call = $ref;
			}
			
			# added to the matrix data
			push(@{$combined_data->{$vcf}}, $base_call);				

			# add to postions output file
			$position_data->{$pos}->{$vcf} = $base_call;
		}

		# added to the matrix data (for reference)
		push(@{$combined_data->{$chrom}}, $ref);				
		
		# add to postions output file (for reference)
...
if ( $refilter ) {
	...
	# iterate over ever position (columns) in the pseudo sequence matrix
	foreach ( my $i = 0; $i < @{$combined_data->{$chrom}}; $i++ ) {
			
		my $ref = $combined_data->{$chrom}->[$i];
		my $alt = '#';  # sets a default value for comparison
		my $flag = 0;
		my $num_n = 0;
		
		# iterate over each sample (rows) in the pseudo sequence matrix
		foreach my $vcf ( keys %{$combined_data} ) {
			...
			# check that each vcf contains correct data
			
			if( !$combined_data->{$vcf}->[$i] ) {
				print "error: ".$vcf."\t".$i."\n";
			}

			# for first iteration set $alt
			$alt = $combined_data->{$vcf}->[$i] if $alt eq '#';

			# increment $flag if the site is different between current and previous vcf
		...
		# if $flag still == 0 then it must be identical in all vcf, therefore mark site with 'X' for removal in split below
		if ( $flag == 0 ) {
			foreach my $vcf ( keys %{$combined_data} ) {
...
##########################################
## Generate PHYLIP and positions file
##########################################

# print header line
print PHYLIP " ".(keys %{$combined_data})." ".@{$combined_data->{$chrom}}."\n";

# print SNP matrix to file
foreach my $vcf ( keys %{$combined_data} ) {
